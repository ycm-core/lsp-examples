...
def Main():
  git_dir = 'systemd-lsp'
  if FindExecutable( 'systemd-lsp' ) is not None and os.path.isdir(os.curdir + os.sep + git_dir) is not True:
    sys.exit( "Error: SystemD Language Server already installed by unknown means" )
  ...
  if os.path.isdir(os.curdir + os.sep + git_dir):
    print("systemd-lsp already cloned into directory")
